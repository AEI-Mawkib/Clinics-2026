/*
 * Mawkib Clinic v3 — runs on ONE Android tablet inside Termux.
  ...
  if (pr) {
    items = db.prepare(
      `SELECT pi.id AS item_id, pi.medicine_id, pi.dosage, pi.dispensed, pi.qty,
              m.name, m.stock FROM prescription_items pi
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/medicines' && req.method === 'POST') {
      ...
      if (b.id) {
        db.prepare('UPDATE medicines SET name = ?, active = ?, stock = ? WHERE id = ?')
          .run(String(b.name || '').trim(), b.active ? 1 : 0,
               Math.max(0, Number(b.stock) || 0), b.id);
      } else if (b.name) {
        db.prepare('INSERT OR IGNORE INTO medicines (name, stock) VALUES (?,?)')
          .run(String(b.name).trim(), Math.max(0, Number(b.stock) || 0));
      }
    ...
    if (disp && req.method === 'POST') {
      ...
      try {
        for (const it of (b.items || [])) {
          ...
          db.prepare('UPDATE prescription_items SET dispensed = ?, qty = ? WHERE id = ?').run(given, qty, row.id);
          if (given) db.prepare('UPDATE medicines SET stock = MAX(stock - ?, 0) WHERE id = ?').run(qty, row.medicine_id);
        }
        db.prepare(
          `UPDATE patients SET status='dispensed', pharmacist_name=?,
             dispensed_at=datetime('now','localtime') WHERE id=?`
        ).run(String(b.pharmacist_name || '').trim(), id);
        db.exec('COMMIT');
      } catch (e) { db.exec('ROLLBACK'); throw e; }
      return json(res, 200, { ok: true });
    }
    ...
    if (p === '/api/stock-summary' && req.method === 'GET') {
      ...
      const rows = db.prepare(
        `SELECT m.id, m.name, m.active, m.stock,
    ...
    if (p === '/api/medicine-report.csv' && req.method === 'GET') {
      ...
      const header = ['Medicine', 'Units dispensed on ' + date, 'Patients served', 'Times not given (stock-out signal)',
      ...
      for (const r of rows) {
        ...
        lines.push([r.name, r.dispensed_today, r.patients_served, r.not_given,
          r.stock, r.stock + r.dispensed_today, status].map(csvCell).join(','));
      }
      const csv = '\uFEFF' + lines.join('\r\n');
      res.writeHead(200, {
        'Content-Type': 'text/csv; charset=utf-8',
        'Content-Disposition': `attachment; filename="medicine-report-${date}.csv"`,
      });
      return res.end(csv);
    }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('==========================================');
  console.log('  Mawkib Clinic v3 is running.');
