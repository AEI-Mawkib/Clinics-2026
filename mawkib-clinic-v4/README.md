# Mawkib Clinic v3 (AEI Telehealth) — offline patient system for the Arbaeen walk
...
git clone https://github.com/YOUR-ORG/mawkib-clinic-v3.git
cd mawkib-clinic-v3
chmod +x start.sh
./start.sh
```
...
- **AEI Telehealth logo** on the top of every page. During your one-time setup (with internet)
  run `./get-logo.sh` once — it downloads the official logo from aeitelehealth.org into
  `public/logo.png`. If the logo file is missing the pages still work, just without it.
