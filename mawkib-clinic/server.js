/*
 * Mawkib Clinic v8 — runs on ONE Android tablet inside Termux.
...
const LIST_TYPES = ['nationality', 'doctor', 'pharmacist', 'site', 'nurse', 'complaint', 'language'];
...
try {
  db.exec("BEGIN");
  db.exec("INSERT INTO list_items (type, name) VALUES ('language', '__probe__')");
  db.exec("ROLLBACK");
} catch (e) {
  ...
  db.exec(`BEGIN;
    ...
    COMMIT;`);
  console.log('list_items table migrated (nurse/complaint/language types).');
  ...
  try {
    ...
    if (p === '/api/patients' && req.method === 'POST') {
      ...
      const info = db.prepare(
        `INSERT INTO patients (token, visit_date, name, age, gender, nationality, language, site, complaint, allergies)
         VALUES (?,?,?,?,?,?,?,?,?,?)`
      ).run(t, today(), name, b.age || null, b.gender || null,
            String(b.nationality || '').trim(), String(b.language || '').trim(),
            getSetting('active_site'),
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('==========================================');
  console.log('  Mawkib Clinic v8 (Azadar e Imam Clinic) is running.');
