/*
 * Mawkib Clinic v9 — runs on ONE Android tablet inside Termux.
...
try {
  db.exec("BEGIN");
  db.exec("INSERT INTO list_items (type, name) VALUES ('intake', '__probe__')");
  db.exec("ROLLBACK");
} catch (e) {
  ...
  db.exec(`BEGIN;
    CREATE TABLE list_items_new (
      ...
      type TEXT NOT NULL CHECK (type IN ('nationality','doctor','pharmacist','site','nurse','complaint','language','intake')),
      name TEXT NOT NULL,
    ...
    COMMIT;`);
  console.log('list_items table migrated (all list types).');
  db.exec(fs.readFileSync(path.join(__dirname, 'schema.sql'), 'utf8')); // re-apply seeds for the new types
}
...
function buildShiftCsv(date) {
  const rows = listPatients(date, 'all');
  const header = ['Token', 'Date', 'Site', 'Registered at', 'Name', 'Age', 'Gender', 'Nationality', 'Language',
    'Complaint', 'Allergies', 'BP', 'Height', 'Weight', 'Temperature', 'Doctor', 'Doctor notes',
    'Medicines prescribed', 'Pharmacist', 'Intake by', 'Wound care', 'Wound care by', 'Status', 'Prescribed at', 'Dispensed at'];
  const lines = [header.join(',')];
  for (const r of rows) {
    lines.push([r.token, r.visit_date, r.site, r.created_at, r.name, r.age, r.gender, r.nationality, r.language,
      r.complaint, r.allergies, r.bp, r.height, r.weight, r.temperature, r.doctor_name, r.notes,
      r.meds, r.pharmacist_name, r.intake_by, r.wound_care, r.wound_care_by, r.status, r.prescribed_at, r.dispensed_at]
      .map(csvCell).join(','));
  }
  return '\uFEFF' + lines.join('\r\n');
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const shiftRoles = ['doctor', 'pharmacist', 'intake'];
    ...
    if (p === '/api/patients' && req.method === 'POST') {
      ...
      const info = db.prepare(
        `INSERT INTO patients (token, visit_date, name, age, gender, nationality, language, intake_by, site, complaint, allergies)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`
      ).run(t, today(), name, b.age || null, b.gender || null,
            String(b.nationality || '').trim(), String(b.language || '').trim(),
            String(b.intake_by || '').trim(), getSetting('active_site'),
    ...
    if (rx && req.method === 'POST') {
      ...
      try {
        ...
        for (const it of (b.items || [])) {
          ...
          const days = Math.min(10, Math.max(1, Number(it.days) || 1));
          const total = doseQty * FREQ_MULT[freq] * days;
          const dosage = `${doseQty} x ${freq} x ${days} day${days > 1 ? 's' : ''} (total ${total})`;
          db.prepare(
            `INSERT INTO prescription_items (prescription_id, medicine_id, dosage, dose_qty, frequency, days, qty)
             VALUES (?,?,?,?,?,?,?)`
          ).run(prId, it.medicine_id, dosage, doseQty, freq, days, total);
        }
        db.prepare(
    ...
    if (p === '/api/analytics' && req.method === 'GET') {
      const date = url.searchParams.get('date') || today();
      const g = db.prepare(
        `SELECT SUM(CASE WHEN age IS NOT NULL AND age < 13 THEN 1 ELSE 0 END) AS children,
                SUM(CASE WHEN gender='M' AND (age IS NULL OR age >= 13) THEN 1 ELSE 0 END) AS male,
                SUM(CASE WHEN gender='F' AND (age IS NULL OR age >= 13) THEN 1 ELSE 0 END) AS female,
                COUNT(*) AS total
           FROM patients WHERE visit_date = ?`).get(date);
      const nat = db.prepare(
        `SELECT COALESCE(NULLIF(nationality,''),'Not recorded') AS k, COUNT(*) AS n
           FROM patients WHERE visit_date = ? GROUP BY k ORDER BY n DESC`).all(date);
      const lang = db.prepare(
        `SELECT COALESCE(NULLIF(language,''),'Not recorded') AS k, COUNT(*) AS n
           FROM patients WHERE visit_date = ? GROUP BY k ORDER BY n DESC`).all(date);
      const times = db.prepare(
        `SELECT (julianday(dispensed_at) - julianday(created_at)) * 1440.0 AS mins
           FROM patients WHERE visit_date = ? AND status='dispensed' AND dispensed_at IS NOT NULL`).all(date)
        .map(r => r.mins).filter(m => m >= 0 && m < 24 * 60);
      const buckets = { '0-15 min': 0, '15-30 min': 0, '30-60 min': 0, '60+ min': 0 };
      for (const m of times) {
        if (m < 15) buckets['0-15 min']++;
    ...
    // ---- patient instructions page (QR target) ----
    if (/^\/rx\/\d+$/.test(p) && req.method === 'GET') {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      return fs.createReadStream(path.join(PUBLIC_DIR, 'rx.html')).pipe(res);
    }
...
// ---------- automatic daily backup (db copy + Excel/CSV dump) ----------
// One file per day, named with the site + date. The hourly refresh keeps the
// current day's files up to date, so the last write of the day is the full day.
const BACKUP_DIR = process.env.BACKUP_DIR || path.join(__dirname, 'backups');
function autoBackup() {
  try {
    fs.mkdirSync(BACKUP_DIR, { recursive: true });
    const site = safeName(getSetting('active_site'));
    const date = today();
    // consistent database snapshot (safe while the clinic is running)
    const dbOut = path.join(BACKUP_DIR, `db-${site}-${date}.db`);
    const tmp = dbOut + '.tmp';
    try { fs.rmSync(tmp, { force: true }); } catch (e) {}
    db.exec(`VACUUM INTO '${tmp.replace(/'/g, "''")}'`);
    fs.renameSync(tmp, dbOut);
    // Excel-compatible dump of the day's patients
    fs.writeFileSync(path.join(BACKUP_DIR, `shift-${site}-${date}.csv`), buildShiftCsv(date));
    console.log(`[auto-backup] saved db + csv for ${date} in ${BACKUP_DIR}`);
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('==========================================');
  console.log('  Mawkib Clinic v9 (Azadar e Imam Clinic) is running.');
